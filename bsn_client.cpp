...
/* 
 * the input argument config must contain the file name of sensor configuration parameters
 * e.g. sensor_config.txt
 */
...
int bsn_set_center1() {
	...
	int failcount = 0;
	int num_samples = 100;
	int sum_samples[MAXSAMP] = {0, 0, 0};
	for (int n = 0; n < num_samples; n++) {
		printf("%d:   %d   %d   %d\n", n, sample1[0], sample1[1], sample1[2]);
		// if all axes are zero, do not sum it up and reset n
		if (sample1[0] == 0 && sample1[1] == 0 && sample1[2] == 0){ 
			n = -1;
			failcount++;
			if (failcount >= 150) {
				center1[0] = 3500;
			...
			for (i = 0; i < MAXSAMP; i++) {
				sum_samples[i] += sample1[i];
			}
		}
	}
	// mean of 100 samples (non-zero)
	for (i = 0; i < MAXSAMP; i++)
		center1[i] = sum_samples[i]/num_samples;
...
int classify_EAR_state(float xp, float yp, float zp) {
	...
	/*
	N      0         0         0
	L - 1.0869    0.7229    0.1653
	R    1.3467 - 0.2434    0.2599
	U    0.2577    1.7360 - 0.2298
	D - 0.2852 - 1.2165    1.2322
	*/
}